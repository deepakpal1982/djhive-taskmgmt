...
export const Tasks = ({ title = 'Tasks', filter = () => true }) => {
  ...
  const { project } = projectApi.endpoints.getProjects.useQuery(undefined, {
    selectFromResult: ({ data }) => ({
      project: data?.find((p) => p.id === parseInt(projectId)),
    }),
  })
  if (Boolean(project)) {
    title = project?.name
  ...
  const { data } = api.endpoints.getTasks.useQuery(undefined, {
    pollingInterval: 0,
  })
  //const { data } = api.endpoints.getTasks.useQuery(undefined)
      ...
      <Container sx={{ mt: (theme) => theme.spacing(2) }}>
        <Paper sx={{ p: 2 }}>
                    ...
                    <TableRow key={task.id}>
                      ...
                      <TableCell
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <Box sx={{ flex: 1 }}>
                            {!Boolean(project) && (
                              <ProjectChip task={task} size='small' />
                            )}{' '}
                            {task.title}
                          </Box>
                          <Box>
          ...
          <Box sx={{ mt: 2 }}>
            <Button
              startIcon={<AddIcon />}
              onClick={() => dispatch(newTask({ project: project }))}>
              Add task
            </Button>
          </Box>
        </Paper>
